...
/*********************************************************
函数名: void UART_Wifi_SEND_IRQHandler(void)
描  述: UART中断服务程序
输入值: 无
输出值: 无
返回值: 无 
**********************************************************/
void UART_Wifi_SEND_IRQHandler(void)  //函数名需要根据实际情况修改，函数内容可能需要自行适配
{
    UART_SendByte(UART5, Uart_WF.uart_txbuf[Uart_WF.uart_tx_count]);
...
/*********************************************************
函数名: void UART_Wifi_RECV_IRQHandler(void)
描  述: UART中断服务程序
输入值: 无
输出值: 无
返回值: 无 
**********************************************************/
void UART_Wifi_RECV_IRQHandler(void) //函数名需要根据实际情况修改，函数内容可能需要自行适配
{
    Uart_WF.uart_rxbuf[Uart_WF.uart_rx_count] = UART_RecByte(UART5);
...
void SendData(unsigned char *str,unsigned char len)
{
    ...
    UART_ITConfig(UART5, UART_IT_TB, ENABLE); //无效发送中断    //使能发送中断，在中断判断并结束发送数据帧   //SetUart5SendStatus();
}

...
void MFS0_UART0_Init(void) //UART0 初始化 9600 打印机通讯 初始化
{
   ...
   stcUart0IrqCb.pfnTxIrqCb = Uart0TxCallback;
   stcUart0IrqCb.pfnRxIrqCb = Uart0RxCallback;
   ...
   Mfs_Uart_DisableFunc(&UART0, UartRx); //Mfs_Uart_EnableFunc(&UART0, UartRx);   //使能接收 @20181130 CFJ
   Mfs_Uart_EnableFunc(&UART0, UartTx);  //使能发送

   Mfs_Uart_DisableIrq(&UART0, UartRxIrq); //无效接受中断//Mfs_Uart_EnableIrq(&UART0, UartRxIrq);  //使能接收中断
...
void Uart2RxCallback(void)
{
	...
	Mfs_Uart_ClrStatus(&UART1,UartOverrunError);
	//RX_TMP= Mfs_Uart_ReceiveData(&UART1);
	
}
 


void MFS1_UART2_Init(void)  //UART0 初始化 9600
{
    stc_mfs_uart_config_t  stcUartConfig;                //UART0
    stc_uart_irq_cb_t      stcUart1IrqCb;
    ...
    stcUart1IrqCb.pfnTxIrqCb  = Uart2TxCallback;
    stcUart1IrqCb.pfnRxIrqCb  = Uart2RxCallback;
    
    stcUartConfig.enMode = UartNormal;
    stcUartConfig.u32BaudRate = 19200;
//    stcUartConfig.u32BaudRate = 9600;
    stcUartConfig.enDataLength = UartEightBits;
    stcUartConfig.enParity = UartParityNone;
    stcUartConfig.enStopBit = UartOneStopBit;
